import pygame
import random
import time

pygame.init()

white=(255, 255, 255)
black=(0, 0, 0)
red=(255, 0, 0)
green=(0, 255, 0)
blue=(0, 0, 255)
light_green=(0, 200, 0)
gray=(119,118,110)

car=pygame.image.load("car.jpg")
car=pygame.transform.scale(car,(110,110))
background=pygame.image.load("background1.jpg")
grass=pygame.image.load("grasline.jpg")


gd=pygame.display.set_mode((800, 600))
clk=pygame.time.Clock()
#pygame.display.update()

def Message_game(size,mess,x_pos,y_pos):
    font=pygame.font.SysFont(None,size)
    render=font.render(mess,True,red)
    gd.blit(render,(x_pos,y_pos))
    #pygame.display.update()

Message_game(50,"START",35,75)
clk.tick(5)
def car_func(x,y):
    gd.blit(car, (x, y))
    gd.blit(grass,(0,0))
    gd.blit(grass,(700,0))
    if 0<x<95 or 700<x+105:
        Message_game(75,"GAME-OVER", 200,150)
        pygame.display.update()
        time.sleep(0.20)
        Game_intro()

def enemy_car(x_r,y_x):
    gd.blit(car,(x_r,y_x))


def button(x_button, y_button, mess_b):
    pygame.draw.rect(gd, green, [x_button,y_button, 100, 30])
    Message_game(50,mess_b,x_button,y_button)
    mouse=pygame.mouse.get_pos()
    click=pygame.mouse.get_pressed()
    if x_button<mouse[0] < x_button+100 and y_button < mouse[1] < y_button+30:
        pygame.draw.rect(gd, light_green, [x_button, y_button, 100, 30])
        Message_game(50, mess_b, x_button, y_button)
        if click == (1, 0, 0) and mess_b == "Play":
            Game_func()
        elif click == (1, 0, 0) and mess_b == "Quit":
            pygame.quit()
            quit()

def car_crash(x,x_r,y,y_r):
    if x_r<x<x_r+90 and y_r<y<y_r+90 or x_r<x+90<x_r+90 and y_r<y<y_r+90:
        Message_game(50,"CRASHED !!",200,150)
        pygame.display.update()
        time.sleep(0.20)
        Game_intro()
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                pygame.quit()
                quit()





def Game_intro():
    intro = False
    while intro == False:
        gd.blit(background, (0, 0))
        button(100,300,"Play")
        button(600,300,"Quit")
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                pygame.quit()
                quit()

        pygame.display.update()


def Game_func():
    x=300
    y=490
    x_change=0
    x_r=random.randrange(100,600)
    y_r=0
    game_over=False
    while game_over==False:
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                game_over=True

            if event.type==pygame.KEYDOWN:
                if event.key==pygame.K_LEFT:
                    x_change=+15


                elif event.key==pygame.K_RIGHT:
                    x_change=-15

            if event.type==pygame.KEYUP:
                if event.key==pygame.K_LEFT or event.key==pygame.K_RIGHT:
                    x_change=0

        gd.fill(gray)
        car_func(x,y)
        enemy_car(x_r,y_r)
        y_r+=10
        if y_r==600:
            x_r = random.randrange(100, 600)
            y_r=0
        car_crash(x,x_r,y,y_r)
        #pygame.draw.rect(gd,red,[x,y,50,50])
        #gd.blit(car,(x,y))
        x=x-x_change
        clk.tick(25)
        pygame.display.update()


#Game_func()
Game_intro()
#Game_func()
pygame.quit()
quit()
